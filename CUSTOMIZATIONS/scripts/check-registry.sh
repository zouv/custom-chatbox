...
# 排除：registry.md 自身、测试文件（测试不强制标记）、node_modules、release 产物
...
while IFS= read -r line; do
  ...
  --include="*.ts" --include="*.tsx" --include="*.yml" --include="*.sh" --include="*.cs" \
  --include=".gitignore" \
  "${PROJECT_ROOT}/src" "${PROJECT_ROOT}/packages" "${PROJECT_ROOT}/CUSTOMIZATIONS" \
  "${PROJECT_ROOT}/.gitignore" "${PROJECT_ROOT}/electron-builder.yml" 2>/dev/null \
...
for file in "${!CODE_MARKERS[@]}"; do
  ...
  for doc in "${!DOC_FILES[@]}"; do
    ...
    # 相对通配行（如 src/renderer/i18n/locales/*/translation.json）
    if [[ "$doc" == *"*"* ]]; then
      wprefix="${doc%%\**}"
...
echo ""
echo "== 2) 总览表登记的 modified-upstream 文件但代码里找不到对应标记 =="
# 仅校验精确路径行；纯自定义路径（上游不存在）与测试文件不要求标记
is_custom_only() {
  case "$1" in
    CUSTOMIZATIONS/*|.agents/*|AGENTS.md|electron-builder.yml|.gitignore|*.test.*|*.test.tsx) return 0 ;;
    *) return 1 ;;
  esac
}
for doc in "${!DOC_FILES[@]}"; do
...
for doc in "${!DOC_FILES[@]}"; do
  if [[ "$doc" == *"（14 个）"* ]]; then
    sample="${PROJECT_ROOT}/src/renderer/i18n/locales/zh-Hans/translation.json"
    if ! grep -q "Auto-Name New Topics" "$sample" 2>/dev/null; then
      echo "  [I18N-KEY-MISSING] zh-Hans/translation.json 缺 Auto-Name 键"
      errors=$((errors+1))
    fi
  fi
done
...
echo ""
if [[ $errors -eq 0 ]]; then
  echo "[OK] 代码标记与改动总览一致（$(echo "${#CODE_MARKERS[@]}") 个含标记文件均已登记）"
  exit 0
else
